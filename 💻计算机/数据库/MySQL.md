...
>- 容易引发数据库的死锁问题
### 🌗唯一约束
>唯一约束用于确保表中的<u>某个列/一组列</u>的值是唯一的
...
唯一约束的作用是防止在指定的列或列组合中插入重复的值。当插入或更新数据时，如果违反了唯一约束，MySQL会抛出错误并拒绝操作。唯一约束可以保证数据的完整性和一致性，并提供了一种简单的方式来确保某些列的值是唯一的。

需要注意的是，唯一约束允许空值（NULL），但是对于包含NULL的多个行，不会违反唯一约束。这意味着在唯一约束的列中，可以有多个NULL值，但是对于非NULL的值，它们必须是唯一的。

总结：在MySQL中，唯一约束用于确保表中的列或列组合的值是唯一的，通过避免插入重复的值来保证数据的完整性。
### 🌗检查约束
