...
redis 在多节点情况下，由于 key 会被 hash 到不同的节点上，而我们使用事务的时候一般都会访问多个 key，事务大概率会失败，所以在多节点情况下，事务几乎没法使用

Redis是一个键值存储系统，它支持单节点事务。然而，在多节点环境中，由于Redis的分布式特性，key可能会被哈希到不同的节点上。这意味着一个事务可能需要在多个节点上进行操作。

但是，由于Redis的事务模型是基于单个节点的，并且没有提供跨节点的事务协调机制，因此在一个事务中对多个节点进行操作会变得非常困难，并且可能导致事务失败。

一种可能的解决方案是在应用程序级别处理事务，但这可能会增加复杂性，并可能引入新的问题。所以总的来说，你说的没错，Redis在多节点环境中使用事务确实会非常困难。
