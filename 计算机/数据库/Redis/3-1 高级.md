...
- 然后，创建一个可读的文件事件用于监听这个客户端 socket 的数据发送

>[!quote] Nagle 算法
>Nagle 算法 通过将小的数据包组合成大的数据包来减少网络传输的次数，可以改善网络传输效率，但是会导致数据延迟

    
3. **创建一个可读的文件事件用于监听这个客户端 socket 的数据发送**：这是 Redis 服务器如何知道何时有数据从客户端发送过来的。当客户端有数据发送到服务器时，这个文件事件会被触发，Redis 服务器就知道可以读取新的数据了。
