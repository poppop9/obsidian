...
> - **条件执行**
> 	- 事务无法条件执行。一旦事务开始，所有的命令都会按照顺序执行。但是在Lua脚本中，你可以使用if-then-else等结构来根据条件执行不同的命令。
> - **性能**：由于Lua脚本在服务器端执行，因此可以避免多次的客户端和服务器之间的网络往返通信，这样可以提高性能。
> - **功能**：Lua脚本可以执行的操作远超过Redis事务。在Lua脚本中，你可以使用各种Lua语言的特性，包括变量、循环和错误处理等。
