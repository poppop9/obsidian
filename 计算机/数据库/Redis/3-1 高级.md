...
一个事务从开始到执行会经历以下三个阶段：
1. **开始事务**：这个阶段是事务的开始，使用 MULTI 命令来标记一个事务的开始，一旦输入 MULTI 命令，Redis 就会开始一个新的事务
2. **命令入队**：在这个阶段，你可以输入你想在事务中执行的所有 Redis 命令。这些命令不会立即执行，而是被放入一个队列中
3. **执行事务**：当你输入 EXEC 命令时，Redis 会一次性执行队列中的所有命令。这就意味着这些命令作为一个原子单元被执行，它们要么全部执行，要么在遇到错误时停止执行。但是，如果在执行事务中的命令时某个命令失败，Redis并不会回滚已经成功执行的命令，而是继续执行剩余的命令。
