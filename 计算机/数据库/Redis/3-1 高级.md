...
# 事务
Redis 的事务提供了一种方式来执行一组命令作为一个单一的操作。这对于需要原子性的操作非常有用。Redis事务的三个重要的保证，具体解释如下：
1. **批量操作在发送EXEC命令前被放入队列缓存**：这意味着，当你开始一个事务时（使用MULTI命令），所有接下来的命令不会立即执行，而是被放入一个队列中。只有当你发送EXEC命令后，所有在队列中的命令才会一次性地执行
2. **收到EXEC命令后进入事务执行，事务中任意命令执行失败，其余的命令依然被执行**：这意味着，如果在执行事务中的命令时，有一个命令失败，那么剩余的命令仍然会被执行。这与许多其他数据库系统的行为不同，它们在一个事务中的命令失败时，通常会回滚整个事务
3. **在事务执行过程，其他客户端提交的命令请求不会插入到事务执行命令序列中**：这意味着，Redis的事务是隔离的。一旦你开始了一个事务，那么直到事务完成，其他客户端的命令不会与事务中的命令混淆
