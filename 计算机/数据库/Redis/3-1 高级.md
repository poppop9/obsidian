...
>[!quote] 乐观锁
>乐观锁 是一种并发控制的方法。它的基本思路是假设多个并发操作之间不会冲突，所以在执行操作的时候不会去获取锁（也就是不会去阻塞其他操作）。只有在提交操作的时候，才会检查是否有冲突。

乐观锁主要用在读操作远远多于写操作的场景，因为在这种情况下，冲突的可能性比较小，不需要在每次读操作时都去获取锁，可以减少锁的开销，提高系统的并发能力。

乐观锁的实现一般是在数据中添加一个版本号字段，每次读取数据时，都会读取版本号。在提交修改时，会检查版本号是否发生改变。如果版本号没有改变，那么就说明这段时间没有其他操作修改这个数据，更新操作可以成功。如果版本号发生了改变，那么就说明有其他操作修改了这个数据，更新操作就会失败。

这就像在一个图书馆里，你在阅读一本书，假设其他人不会修改这本书。只有在你要写入自己的笔记时，才会检查这本书是否被其他人修改过。如果没有被修改，你就可以写入你的笔记。如果被修改了，你可能就需要重新阅读这本书，再写入你的笔记。
