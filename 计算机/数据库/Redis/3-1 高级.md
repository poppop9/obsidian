...
Redis是一个类似于超级"储物箱"的系统，其中的每一个"箱子"（我们称之为key）都有一个唯一的名字，并且里面可以放任何东西（我们称之为value）。

现在，假设你有一个大仓库（单节点），你可以很方便地在里面找到你需要的"箱子"，或者同时处理很多"箱子"（这就是所谓的事务）。

但是，如果你的"箱子"太多了，一个仓库放不下，你需要分成多个仓库（多节点）来存放。这时，你需要一个方法来决定哪个"箱子"放在哪个仓库，这就是哈希。比如，你可能会根据"箱子"名字的字母来决定，A-M放在一号仓库，N-Z放在二号仓库。

这样做的问题是，如果你想同时处理很多"箱子"（事务），可能会发现需要的"箱子"分散在不同的仓库，你就需要在多个仓库之间来回跑，这就变得很麻烦，可能会导致事务处理失败。这就是为什么在多节点环境下，使用Redis的事务会很困难。
