...
> 	- `列表 List`：字符串有序列表，支持索引，<u>按照插入顺序排序</u>【可以头插，尾插】。可存储 40 多亿元素
> 	- `集合 Set`：字符串无序列表，集合中不允许有重复值，通过哈希表实现
> 	- `有序集合 ZSet`：字符串集合，且不允许重复的成员，每个元素都会关联一个 double 类型的分数【~~分数可以重复~~】，通过分数来为集合中的成员进行<u>从小到大</u>的排序
> 	- `哈希 Hash`：是一个键值对集合，可存储 40 多亿个键值对，**适合存储对象**
> - **高级数据类型**
> 	- 消息队列 Stream
> 	- 地理空间 Geospatial
> 	- HyperLogLog
...
## 位图 Bitmap
>[!quote] 位图
>位图 就是字符串，只不过这个字符串是二进制形式的，这样我们可以<u>更加省内存地</u>记录 bool 类型的值【~~按照 String 可以存储 512 MB 来算，如果要记录一个用户一年的签到次数，正常来说需要 365 个键值对，而只用使用一个位图，就可以记录 2^32 次的签到次数~~】
