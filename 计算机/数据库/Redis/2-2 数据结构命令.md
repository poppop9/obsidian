...
>[!hint] Stream 还支持消费者组。消费者组是一种可以被多个消费者共享的数据流，每个消费者可以读取流中的记录，并且记录消费者已经读取了哪些记录。这使得 Stream 可以实现分布式的消息消费，即多个消费者可以同时从同一个 Stream 中读取和处理消息，提高了消息处理的效率
...
graph LR
    ...
    subgraph ConsumerGroup2
        MessageContent[Message Content]
    end
    ConsumerGroup1 -->|Last_delivered_id| StreamDirection
    StreamDirection -->|Pending_ids[]| ConsumerGroup2
```
