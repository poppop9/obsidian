...
## 位图 Bitmap
>[!quote] 位图
>位图 就是字符串，只不过这个字符串是二进制形式的，这样我们可以<u>更加省内存地</u>记录 bool 类型的值【~~如果要记录一个用户一年的签到次数，正常来说需要 365 个键值对，而只用使用一个位图，就可以记录 2^32 次的签到次数~~】
...
然而，你提供的字符串"11110000"是一个包含8个字符的字符串，而不是一个二进制的位串。在ASCII编码中，每个字符占用8个比特位，所以一个8个字符的字符串实际上包含了64比特位。这就是BITCOUNT返回20的原因。

  

如果你想要计算一个二进制位串中的1的数量，你应该直接传入二进制位串，而不是字符串。例如，BITCOUNT(11110000)的结果就是4，因为这个二进制位串中包含4个1。

  

希望这个解释可以帮助你理解BITCOUNT函数的工作方式。如果你还有其他问题，欢迎继续提问。