...
# 磁盘管理
>[!hint] `cp` 和 `dd` 的比较
> `cp` 只复制文件内容，而<u>不复制</u>【创建时间，修改时间……】，也<u>不复制文件系统的特殊结构</u>【硬链接和软链接（快捷方式），设备文件】

>[!hint] `cp` 不能复制哪些文件 ？
>- 元数据

1. ****：这些是文件系统中的特殊类型的文件，它们实际上是指向其他文件的引用。`cp`命令通常会复制链接指向的文件，而不是链接本身，而`dd`可以复制链接本身。
2. **设备文件**：在Unix和类Unix系统中，设备文件是对系统中物理设备（如硬盘、键盘、鼠标等）的抽象。`cp`命令通常无法正确处理设备文件，而`dd`可以。
    
3. **文件权限和所有权**：`cp`命令通常会按照当前用户的权限来创建新文件，而不会保留原文件的权限和所有权。而`dd`可以复制文件的权限和所有权。
    
4. **文件系统元数据**：这包括文件的创建时间、修改时间、最后访问时间等。`cp`命令通常不会保留这些元数据，而`dd`可以。
    
5. **磁盘的引导扇区和分区表**：这些是磁盘的特殊区域，包含了操作系统启动和磁盘分区的信息。`cp`命令无法复制这些区域，而`dd`可以。

`dd` 命令则更为强大，它可以按照指定的块大小直接复制磁盘的原始内容，而不仅仅是文件的内容。这使得 `dd` 能够复制文件系统结构、引导区、空文件等，这是 `cp` 命令无法做到的。

由于 `dd` 命令的这种特性，它常常用于制作或复制磁盘镜像、备份和恢复整个磁盘或分区、复制磁盘的某个区域等任务。

在 Linux 上，硬件的设备驱动【硬盘……】，和特殊设备文件【`/dev/zero`， `/dev/random`】也是文件，`dd` 也可以读取文件或写入这些文件。这样，`dd` 也可以用在备份硬件的引导扇区、获取一定数量的随机数据或者空数据等任务中。`dd` 程序也可以<u>在复制时处理数据</u>【转换字节序，在 ASCII 与 EBCDIC 编码间互换……】
