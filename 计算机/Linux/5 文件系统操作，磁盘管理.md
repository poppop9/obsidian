...
一个目录被用作挂载点，原来在这个目录下的文件在挂载期间将无法被访问，直到文件系统被卸载

外部设备可以分为两种类型：字符设备和块设备

块设备是一种数据存储设备，它以一定大小的数据块为单位进行数据的读写。例如硬盘、USB驱动器、CD-ROM等都是块设备。这些设备的特点是可以随机访问存储介质上任意位置的数据，并且可以进行缓存操作。

块设备与字符设备的主要区别在于，字符设备是以字节流方式进行数据传输，而块设备则是以一块一块的数据为单位进行数据传输。比如，键盘和鼠标就是字符设备，它们的输入输出是顺序的，不能进行随机访问。

在操作系统中，块设备通常会被表示为一个设备文件，例如硬盘通常会被表示为`/dev/sda`，`/dev/sdb`
