...
`dd` 默认从标准输入【键盘】中读取数据，并将数据写入到标准输出（通常是屏幕）。但是，我们通常不会这样使用 `dd` 命令，因为这样并不能发挥出 `dd` 的强大功能。

我们通常会使用 `if`（input file，输入文件）和 `of`（output file，输出文件）选项来告诉 `dd` 从哪个文件读取数据，以及将数据写入到哪个文件。
