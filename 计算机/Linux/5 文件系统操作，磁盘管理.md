...
- 使用 `mkfs` 【~~make filesystem~~】将虚拟磁盘镜像格式化【将<u>文件系统</u>（ext4……）写入到虚拟磁盘镜像中】
```bash
# 将 "virtual.img" 文件格式化为 ext4 文件系统
mkfs -t ext4 virtual.img
```

这样，你就可以把 "virtual.img" 文件当作一个拥有 ext4 文件系统的虚拟磁盘来使用了。你可以把它挂载到你的系统中，然后在其上创建文件和目录，就像在一个真实的磁盘上操作一样。
