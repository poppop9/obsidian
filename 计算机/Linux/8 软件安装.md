...
## 清除软件包
>[!hint] `/var/cache/apt/archives/` 下存放了所有已下载的软件包文件，这些包文件在安装或升级完成后，这些文件就没有用处了，所以使用 `apt-get clean` 可以帮助你清理硬盘空间。

 `apt-get clean` 移除在 `/var/cache/apt/archives/` 目录下的所有已下载的软件包文件。这些文件是在你使用 `apt-get install` 或 `apt-get upgrade` 命令安装或升级软件时下载的。在安装或升级完成后，这些文件就没有用处了，所以使用 `apt-get clean` 可以帮助你清理硬盘空间。
    
`apt-get autoclean` 移除已经被安装或升级的软件的旧版本软件包文件。也就是说，如果一个软件有新的版本被安装或升级，那么这个软件旧版本的软件包文件就会被 `apt-get autoclean` 移除。这个命令相比 `apt-get clean` 更加保守，它不会移除所有的软件包文件，只会移除不再需要的旧版本软件包文件
