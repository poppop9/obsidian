...
>[!hint] 如果不使用 next() 方法，直接使用 previous() 方法，会导致什么也不打印。因为实现遍历的背后原理是指针。没有向后遍历的基础，向前遍历指针指向集合的最开始，所以不会有结果
...
以下代码会报出异常，这是由于在迭代时使用 List 集合的 `add` 方法改变集合导致的（因为这样不好定义迭代结果），如果一定要在迭代时改变集合元素，请使用 ListIterator 的 `add` 方法
```java
public static void main(String[] args) {  
    List<String> list = new ArrayList<>();  
