...
## 线程同步
>[!hint] 由于 `HashMap` 是非线程安全的，当多个线程并发访问和修改 `HashMap`，需要外部同步

灵活性：允许一个null键和多个null值。
迭代器：提供了快速失败的迭代器，可以在迭代过程中检测到并发修改。
缺点：

>[!quote] 提供线程安全的方法
>- `HashTable` 不推荐
>	- 迭代器是弱一致的
>- `Collections.synchronizedMap()` 不推荐
...
### HashTable
>[!quote] HashTable
>HashTable 在内部实现了线程安全
>
> - `HashTable` 不允许键和值为`null`

>[!hint] 不过我们需要线程安全时一般使用  synchronizedMap() 方法
> ```java
...
`ConcurrentHashMap` 使用分段锁的概念，这意味着它将 Map 分成多个段，每个段可以独立地被锁定和访问。这提供了更好的并发性能和可伸缩性
