...
## Set
>[!quote] Set
>Set 集合 <u>不能有重复元素</u>，<u>可以有 `NULL` 值</u>，<u>无序</u>，<u>没有索引</u>
> ```
...
细粒度锁：
相对于 Hashtable 的粗粒度锁（整个集合加一个锁），ConcurrentHashMap 的锁粒度更细，每个段一个锁，这样提高了并行度。

高性能：
由于内部实现了细粒度的锁和无锁设计，ConcurrentHashMap 在多线程环境下的性能通常优于使用粗粒度锁的 Hashtable。

透明性：
作为使用者，你不需要关心 ConcurrentHashMap 的同步细节，它提供了线程安全的Map实现，可以像使用普通 HashMap 一样使用它。
迭代器安全性：

ConcurrentHashMap 提供的迭代器是弱一致的，这意味着它们提供了对集合的一个瞬时快照，并且可以容忍迭代过程中的并发修改。
因此，可以说 ConcurrentHashMap 是一种内部同步机制，因为它封装了所有同步逻辑，并提供了线程安全的访问，而不需要使用者进行额外的同步操作。这种设计使得 ConcurrentHashMap 成为现代Java并发编程中推荐使用的线程安全Map实现。

---



线程安全性：通过分段锁技术实现线程安全，提供了更好的并发性能。
性能：在多线程环境下，性能优于同步包装的HashMap和HashTable。
灵活性：允许null键和null值。
迭代器：提供了弱一致的迭代器，可以在迭代过程中安全地处理Map的并发修改。
缺点：

复杂性：实现比HashMap和HashTable复杂，这可能导致更高的内存占用。
性能开销：虽然比HashTable好，但在单线程环境下，性能仍然不如非线程安全的HashMap。
总结
HashMap：适用于单线程环境或需要快速插入和查找的场景，需要外部同步来保证线程安全。
HashTable：适用于需要线程安全的遗留代码，但由于性能问题，通常被推荐使用ConcurrentHashMap替代。
ConcurrentHashMap：适用于多线程环境，需要高并发性能的场景，提供了更好的线程安全性和并发性能。
