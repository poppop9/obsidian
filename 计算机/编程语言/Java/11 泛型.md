...
- 编译后的代码差异
第一段代码中的`getData`方法使用了泛型方法`<T>`，这意味着对于每个不同的泛型实参，方法都能接受特定类型的`List`。在编译后的代码中，所有的`T`都会被擦除，并替换为`Object`，因为`T`是无界的。第二段代码中的`getData`方法使用了通配符`?`，这表示方法可以接受任何类型的`List`。在编译后的代码中，通配符`?`同样会被擦除，并且方法参数会被视为`List`类型，而不是`List<Object>`，因为通配符不是具体类型
...
总结来说，两段代码在编译后的字节码中都会丢失具体的泛型类型信息，因为Java使用类型擦除来实现泛型。在运行时，两者的行为相同，因为泛型信息已经被擦除。在类型安全性方面，两者都能在编译时提供类型检查，防止将错误类型的对象添加到集合中。
