...
- 文件类型
	...
	- `socket`：网络套接字
	- `pipe`： 管道
	- `软链接文件`：软链接等同于 Windows 上的快捷方式，另一种是“硬链接”
...
当你想要打开一个目录并查看其内部的文件时，该目录必须同时具有读权限和执行权限。读权限（read permission）允许你读取目录的内容，也就是查看目录中的文件列表和相关信息。执行权限（execute permission）允许你进入目录并访问其内部的文件。

目录在文件系统中被视为一种特殊类型的文件。除了保存实际文件的数据外，目录文件还保存着该目录内部的文件列表和其他相关信息，如文件名、文件的inode号等
