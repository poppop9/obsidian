...
对于内联元素，`text-align: center;` 属性只能影响其内部文本内容的对齐方式，并不能使整个元素在父容器中水平居中。要使链接水平居中，你需要将其放置在一个具有居中对齐的父容器中，或者使用其他布局技术（如Flexbox或Grid）来实现居中对齐。

对于块级元素（如段落），`text-align: center;` 属性可以将其内部文本内容和自身在父容器中水平居中对齐，因此你可以看到段落中的文本内容是居中显示的。
