...
>- `VO`【~~value object~~】 VO 用于业务层之间的数据传递，和PO一样也是仅仅包含数据而已。但应是抽象出的业务对象，可以和表对应，也可以不，这根据业务的需要。VO对象通常只包含要用到的数据属性，不包含业务逻辑
>- `DAO` 
>- `BO` 
>- `DTO` 
>- `POJO` 
...
>[!hint] 在领域与领域之间，如果需要某个充血模型，要把充血模型使用工厂组装成贫血模型进行传输
