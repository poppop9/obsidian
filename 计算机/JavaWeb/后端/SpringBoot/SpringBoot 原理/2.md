...
Bean 定义是用来描述一个 Bean 的各种属性的【Bean 的类名，Bean 的作用域，Bean 的构造函数参数，Bean 的属性值，Bean 的初始化方法，销毁方法……】

一旦 `ApplicationContext` 已经刷新，Bean 的定义就不能再被修改了

但是，你应该尽量避免在运行时修改 Bean 的定义，而是在配置阶段就确定好 Bean 的定义
