...
---
`if(event instanceof ApplicationReadyEvent)` 这段代码是在检查当前的事件对象（`event`）是否是 `ApplicationReadyEvent` 类型。

  

在Java中，`instanceof` 是一个关键字，用来检查一个对象是否是某个类的实例。语法为： `object instanceof ClassName`，如果 `object` 是 `ClassName` 类型的实例，那么这个表达式就会返回 `true`，否则返回 `false`。

  

`ApplicationReadyEvent` 是Spring Boot提供的一个事件类型，这个事件会在Spring Boot应用启动完成并且所有的Spring Bean都已经创建完毕后被触发。也就是说，当你的应用启动并且准备好接受请求时，Spring Boot会发布一个 `ApplicationReadyEvent` 事件。

  

所以，当你在事件监听器中写 `if(event instanceof ApplicationReadyEvent)` 时，意思是你想要在应用启动完成后执行一些特定的操作。如果当前的事件是 `ApplicationReadyEvent`，那么就执行括号内的代码，否则这段代码就会被忽略。
